...
<ResponseField name="modifier" type="'alt' | 'ctrl' | 'meta' | 'shift'" default="meta">
	Sets global key modifier used to detect select alhttps://www.artstation.com/artwork/9EkOANl and
	duplicate events.
</ResponseField>
...
<ResponseField name="on:connection">
	Fires when the user drags an Edge which successfully connects two anchors.
</ResponseField>

<ResponseField name="on:disconnection">
	Fires when the user disconnects an edisting Edge between two anchors.
</ResponseField>
